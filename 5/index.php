...
function pass($string){
    if(strlen($string)<=10){
       return "Пароль должен быть больше 10 символов.";
        ...
        print('В пароле должно быть меньше 3 букв нижнего регистра подряд.');
    }
    else if(preg_match('{^(.*[A-Z].*){4,}$}', $string)) {
        print('В пароле должно быть меньше 3 букв верхнего регистра подряд.');
    }
    else if(preg_match('{^(.*[0-9].*){4,}$}', $string)) {
        print('В пароле должно быть меньше 3 цифр подряд.');
    }
    else if(preg_match('{^(.*[%,$,#,_,*].*){4,}$}', $string)) {
        print('В пароле должно быть меньше 3 специальных символов подряд.');
    }
