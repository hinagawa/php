...
<script>
    function likes() {
        <?php $check = mysqli_query($mysqli, "SELECT * FROM `user_like` WHERE `news_id` = '$id' and `user_id`='$user'" );
        if (mysqli_num_rows($check) > 0) {
            $x=$x;
            }
        else
        {$x= $x+1;
            mysqli_query($mysqli, "UPDATE news SET likes=$x WHERE idnews=$id");
            mysqli_query($mysqli, "INSERT INTO `user_like` (`news_id`, `user_id`) VALUES ('$id','$user') ");
